...
function run(inputs) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      const deploy = yield netlifyClient.deploy(siteId, deployFolder, __spreadProps(__spreadValues({
      ...
      const message = productionDeploy ? `\u{1F389} Published on ${deploy.deploy.ssl_url} as production
\u{1F680} Deployed on ${deploy.deploy.deploy_ssl_url}` : `\u{1F680} Deployed on ${deploy.deploy.deploy_ssl_url}`;
...
${message}`;
      ...
      try {
        const environment = (_a = inputs.githubDeploymentEnvironment()) != null ? _a : productionDeploy ? "production" : import_github.context.issue.number !== void 0 ? "pull request" : "commit";
      ...
      if (inputs.enableCommitStatus()) {
        try {
          const sha = (_c = (_b = import_github.context.payload.pull_request) == null ? void 0 : _b.head.sha) != null ? _c : import_github.context.sha;
