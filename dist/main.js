...
// node_modules/any-shell-escape/shell-escape.js
var require_shell_escape = __commonJS({
  "node_modules/any-shell-escape/shell-escape.js"(exports, module2) {
    var util = require("util");
    ...
    module2.exports = function(stringOrArray) {
      var ret = [];
    ...
    if (winCmd) {
      module2.exports.msg = function(x) {
        if (typeof x == "string") {
          return x;
        } else {
          return x.join(" ");
      ...
      module2.exports.msg = module2.exports;
    }
  }
});

// node_modules/shellsync/dist/signals.js
var require_signals = __commonJS({
  "node_modules/shellsync/dist/signals.js"(exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    var shellEscape = require_shell_escape();
    var isActive = false;
    var options = { timeout: null };
    var signalReceived;
    ...
    var handleSignalInt = () => handleSignal("SIGINT");
    var handleSignalTerm = () => handleSignal("SIGTERM");
    var handleSignalQuit = () => handleSignal("SIGQUIT");
    ...
    function wrapDisableInterrupts(script) {
      return `:
        ...
        triggerTimeout() {
            local PID=$$
            ${options.timeout ? `(sleep ${Math.round(options.timeout / 1e3)}; kill -SIGUSR1 $PID 2>/dev/null) &` : ``}
        }
        onTimeout() {
...
// node_modules/shellsync/dist/mocking.js
var require_mocking = __commonJS({
  "node_modules/shellsync/dist/mocking.js"(exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    var metaStream = 3;
    var mockStream = 4;
...
// node_modules/shellsync/dist/index.js
var require_dist = __commonJS({
  "node_modules/shellsync/dist/index.js"(exports, module2) {
    "use strict";
    var child_process = require("child_process");
    var fs_1 = require("fs");
    var signals_1 = require_signals();
    var mocking_1 = require_mocking();
    var npmRunPath = require_npm_run_path();
    var shellEscape = require_shell_escape();
    var bashBuiltinError = 2;
    var metaStream = 3;
    var mockStream = 4;
    var stdioDefault = [0, "pipe", "inherit", "pipe", "pipe"];
    var stdioHushed = [0, "pipe", "pipe", "pipe", "pipe"];
    var stdioOut = [0, "inherit", "inherit", "pipe", "pipe"];
    var ParseState;
    (function(ParseState2) {
      ParseState2["BackTick"] = "BackTick";
      ParseState2["Expression"] = "Expression";
      ParseState2["DoubleQuoted"] = "DoubleQuoted";
      ParseState2["SingleQuoted"] = "SingleQuoted";
    ...
    function createShell(options, mocks = new mocking_1.MockManager()) {
      ...
      options.preferLocal = options.preferLocal === void 0 ? true : options.preferLocal;
      const exec = (overrideOptions, commands, ...commandVars) => {
        ...
        if (output && output[metaStream].startsWith("\0\0"))
          ...
          shell2.options.cwd = output[metaStream];
...
${parse.stderr}`;
      ...
      return Object.assign(execOrCreateShell, shell2);
    }
    var quote = (commands, ...commandVars) => {
      ...
      for (var i = 0; i < fragment.length; i++) {
        ...
        switch (char) {
          ...
          case "`":
            ...
            break;
          case '"':
            process2(DoubleQuoted);
            break;
          case "'":
            if (last() === DoubleQuoted)
              break;
            process2(SingleQuoted);
            break;
          case "$":
            if (fragment[i + 1] !== "(")
              break;
    ...
    var sharedMocks = new mocking_1.MockManager();
    var shell = createShell({}, sharedMocks);
    var shellHushed = createShell(Object.assign({}, shell.options, { stdio: stdioHushed }), sharedMocks);
    var sh2 = Object.assign(shell, {
      sh: shell,
      shh: shellHushed,
      quote,
      unquoted,
...
var import_shellsync = __toESM(require_dist());
...
var defaultInputs = {
  ...
  functionsDir() {
    return core.getInput("functions-dir") || void 0;
  },
  deployMessage() {
    return core.getInput("deploy-message") || void 0;
  },
  productionBranch() {
    return core.getInput("production-branch") || void 0;
  },
  productionDeploy() {
  ...
  netlifyConfigPath() {
    return core.getInput("netlify-config-path") || void 0;
  },
  alias() {
    const alias = core.getInput("alias");
    if (alias && alias.trim()) {
      return encodeURIComponent(alias.toLowerCase().replace(/[^a-z0-9 _-]+/gi, "-"));
    } else {
      return void 0;
    }
  },
  githubDeploymentEnvironment() {
    return core.getInput("github-deployment-environment") || void 0;
  },
  githubDeploymentDescription() {
    return core.getInput("github-deployment-description") || void 0;
  },
...
function getCommentIdentifier(siteId) {
  const sha256SiteId = crypto.createHash("sha256").update(siteId).digest("hex");
...
function findIssueComment(githubClient, siteId) {
  return __async(this, null, function* () {
    var _a;
    const listCommentsRes = yield githubClient.rest.issues.listComments({
    ...
    for (const comment of comments) {
      if ((_a = comment.body) == null ? void 0 : _a.includes(commentIdentifier)) {
        return comment.id;
      }
    }
    return void 0;
...
function createGitHubDeployment(githubClient, environmentUrl, environment, description) {
  return __async(this, null, function* () {
    var _a, _b;
    const deployRef = (_b = (_a = import_github.context.payload.pull_request) == null ? void 0 : _a.head.sha) != null ? _b : import_github.context.sha;
    const deployment = yield githubClient.rest.repos.createDeployment({
      auto_merge: false,
    ...
    if (deployment.status === 201) {
      yield githubClient.rest.repos.createDeploymentStatus({
        state: "success",
...
function run(inputs) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      const functionsDir = inputs.functionsDir();
      ...
      {
        const response = import_shellsync.sh`npm install netlify-cli -g`;
      }
      const deployResult = import_shellsync.sh.array`npx netlify deploy --dir=${deployFolder} --message=${deployMessage != null ? deployMessage : "From NetlifyDeploy"} ${deployMode}`;
      if (productionDeploy && alias !== void 0) {
        console.warn(`Only production deployment was conducted. The alias ${alias} was ignored.`);
      }
      console.info(deployResult);
      const deploy = ((_a = deployResult[5].split(" ")) == null ? void 0 : _a[0]) ? { production: deployResult[5].split(" ")[8] } : { staging: deployResult[4].split(" ")[3] };
      const deployURL = (_b = deploy.production) != null ? _b : deploy.staging;
      const message = "production" in deploy ? `\u{1F389} Production deployed on ${deploy.production}` : `\u{1F680} Staging Deployed on ${deploy.staging}`;
      process.stdout.write(`${message}
`);
      core2.setOutput("deploy-url", deployURL);
...
${message}`;
      const githubClient = (0, import_github.getOctokit)(githubToken);
      if (enableCommitComment) {
        ...
        try {
          yield githubClient.rest.repos.createCommitComment(commitCommentParams);
        } catch (err) {
          console.error(err, JSON.stringify(commitCommentParams, null, 2));
        }
      }
      if (import_github.context.issue.number !== void 0) {
      ...
      try {
        const environment = (_c = inputs.githubDeploymentEnvironment()) != null ? _c : productionDeploy ? "production" : import_github.context.issue.number !== void 0 ? "pull request" : "commit";
      ...
      if (inputs.enableCommitStatus()) {
        try {
          const sha = (_e = (_d = import_github.context.payload.pull_request) == null ? void 0 : _d.head.sha) != null ? _e : import_github.context.sha;
          yield githubClient.rest.repos.createCommitStatus({
            owner: import_github.context.repo.owner,
            repo: import_github.context.repo.repo,
            context: "Netlify",
      ...
      if (error instanceof Error) {
        ...
        core2.setFailed("Error in actions-netlify and Error wasn't an instance of Error");
      }
    }
  });
}
